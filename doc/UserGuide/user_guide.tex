...
transitions essentially ``for free''. This can be suppressed by setting \hyperref[keyword:--no-extra-transitions]{{-}{-}no-extra-transitions}.
...
\texttt{--no-diagonalisation}
\begin{adjustwidth}{1cm}{}
Do not diagonalise CI matrix, just generate and write the matrices to a file. Useful if you want to use a different run setup for generating and solving. \ambit\ always checks to see if \texttt{*.matrix} files exist before generating, so simply running again without this flag will move directly to the diagonalisation stage.
\end{adjustwidth}
...
\texttt{{-}{-}no-extra-transitions} \label{keyword:--no-extra-transitions}
\begin{adjustwidth}{1cm}{}
Suppress the calculation of additional transitions outside of the range specified by MatrixElements or AllBelow. This can reduce the output file size and computational time, which is only useful in calculations with an extremely large number of levels.
\end{adjustwidth}
